...
const selectPropertiesWithoutVideo = `
  id,
  title,
  type,
  price,
...
const selectPropertiesWithoutFloorPlan = `
  id,
  title,
  type,
  price,
...
const selectPropertiesWithoutVideoAndFloorPlan = `
  id,
  title,
  type,
  price,
...
async function fetchProperties(queryBuilder) {
  ...
  if (isMissingFloorPlanColumn(error)) {
    const fallback = await queryBuilder(selectPropertiesWithoutFloorPlan);
    data = fallback.data;
    error = fallback.error;
  }

  if (isMissingVideoColumn(error)) {
    const fallback = await queryBuilder(selectPropertiesWithoutVideo);
    data = fallback.data;
    error = fallback.error;
  }

  if (isMissingFloorPlanColumn(error)) {
    const fallback = await queryBuilder(selectPropertiesWithoutVideoAndFloorPlan);
    data = fallback.data;
    error = fallback.error;
  }
